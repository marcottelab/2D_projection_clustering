...
with open('./' + out_dir_orig + '/train_clusters.txt','r') as f:
    train_clusters = f.readlines()
    
train_clusters = [line.rstrip().split() for line in train_clusters]
    
out_dir_orig = out_dir_orig + 'train_siamese'
...
siamese_pairs = []
siamese_triples_with_sampled_negs = [] # Format: anchor, positive, negative

# Starting from index 0

for cluster_set in train_clusters:
    cluster_pairs = list(combinations(cluster_set, 2))
    siamese_pairs = siamese_pairs + cluster_pairs
    remn_clusters = train_clusters
    remn_clusters.remove(cluster_set)
    
    for pair in cluster_pairs:
        sampled_negatives = []
        for neg_clust in remn_clusters:
            sampled_negatives.append(random.choice(neg_clust))
            
        siamese_triples_with_sampled_negs = siamese_triples_with_sampled_negs + [(pair[0],pair[1],str(neg)) for neg in sampled_negatives]
...
#anchor_images = [data[int(triple[0])] for triple in siamese_triples_with_sampled_negs]
anchor_images = [converted_data[int(triple[0])] for triple in siamese_triples_with_sampled_negs]

logger.info('Making positive images list...')        
#positive_images = [data[int(triple[1])] for triple in siamese_triples_with_sampled_negs]
positive_images = [converted_data[int(triple[1])] for triple in siamese_triples_with_sampled_negs]
...
tf.data.experimental.save(train_dataset,'train_dataset_preprocessed_more_negs.tf')
tf.data.experimental.save(val_dataset,'val_dataset_preprocessed_more_negs.tf')
...
try:
    with open('siamese_embedding_model_preprocessed_more_negs.pkl','wb') as f:
        pickle.dump(embedding,f)
except:
    embedding.save('siamese_embedding_model_preprocessed_more_negs.tf')
