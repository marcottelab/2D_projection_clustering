...
def get_projection_2D(mrcs, factor,out_size=(200,200),resize=False,pad=0):
    ...
    for k, avg in projection_2D.items():
        ...
        if factor == 1:
            resized_image = extract_class_avg(resized_image)
        else:
            ...
            resized_image = extract_class_avg(scaled_img)
            
        if pad:
            resized_image = resize_with_crop_or_pad(resized_image[:,:,newaxis],pad,pad)
            resized_image = np.squeeze(np.array(resized_image))
        if resize:
            resized_image =  transform.resize(resized_image, out_size)  
            
        projection_2D[k] = resized_image.copy()
